...
export async function POST(req: Request) {
  try {
    const { candidateName, candidateEmail, assessmentName, assessmentLink } = await req.json();
    
    if (!candidateName || !candidateEmail || !assessmentName || !assessmentLink) {
    ...
    await sendEmail({
        ...
        subject: `Your assessment for ${assessmentName} has been reset`,
        htmlBody: template,
    });
